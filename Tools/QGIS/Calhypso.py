...
##Id_field=field Basin_shapefile
##Name_field=field Basin_shapefile
...
class CurvaHypsometrica:
    ...
    def get_data(self):
        """
        Return the curve data as an array of 2 columns
        """
        return self._data
...
class CalHypsoApp:
    ...
    def __init__(self, curves, figure, cm="BrBG", hi_colors=True, basedir=""):
        ...
        self.saving = False
        self.info_text = figure.text(0.74, 0.9, "", size=11, verticalalignment="top")
    ...
    def format_axe(self, ax, labels=True, grids=True):
        ...
        :param ax: (Axe) Matplotlib Axe to format
    ...
    def draw_active(self):
        ...
        ax.plot(curva.get_aa(), curva.get_hh(), c="navy", lw=1.5)    
        ax.text(0.82, 0.97, cadena, size=11, verticalalignment="top")
        self.gr_type = 1
        ax.figure.canvas.draw()
...
curvas = get_hypsometric_curves(dem, basin_shapefile, attr_field, name_field)
figure = plt.figure(figsize=(8.5, 6.))
calypso = CalHypsoApp(curvas, figure, basedir=basedir)
plt.show()
